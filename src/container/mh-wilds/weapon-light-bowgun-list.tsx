...
import { mhWildsLightBowgunsData } from "@/data/mh-wilds";
import { NoResults } from "@container/common/no-results";
import { usePagination } from "@/hook/use-pageation";
import { Pagination } from "@infrastructure/common/pagenation";

export function LightBowgunList({ searchTerm }: { searchTerm: string }) {
  const { getNamespaceData } = useI18n();

  const mhCommonNamespace = getNamespaceData("mh_common");
  const mhWildsLightbowgunNamespace = React.useMemo(
    () => getNamespaceData("mhWilds_light_bowguns") ?? {},
    [getNamespaceData]
  );
  ...
  const filteredList = React.useMemo(() => {
    return mhWildsLightBowgunsData.filter(
      ({ name, rank, rarity }) =>
        (!selectedRank || rank === selectedRank) &&
        (!isFinalOnly || rarity === 4 || rarity === 8) &&
        (mhWildsLightbowgunNamespace[name] ?? name)
          .toLowerCase()
          .includes(searchTerm.toLowerCase())
    );
  }, [selectedRank, isFinalOnly, mhWildsLightbowgunNamespace, searchTerm]);
              ...
              <div key={name} className="border p-4 rounded shadow space-y-2">
                <div className="flex items-center">
                  <div className="font-semibold">
                    {mhWildsLightbowgunNamespace[name]}
                  </div>
                  {slots.length > 0 && (
