...
import { mhWildsLongswordsData } from "@/data/mh-wilds";
import { NoResults } from "@container/common/no-results";
import { usePagination } from "@/hook/use-pageation";
import { Pagination } from "@infrastructure/common/pagenation";

export function LongswordList({ searchTerm }: { searchTerm: string }) {
  const { getNamespaceData } = useI18n();

  const mhCommonNamespace = getNamespaceData("mh_common");
  const mhWildsLongswordNamespace = React.useMemo(
    () => getNamespaceData("mhWilds_longswords") ?? {},
    [getNamespaceData]
  );
  ...
  const filteredList = React.useMemo(() => {
    return mhWildsLongswordsData.filter(
      ({ name, rank, rarity }) =>
        (!selectedRank || rank === selectedRank) &&
        (!isFinalOnly || rarity === 4 || rarity === 8) &&
        (mhWildsLongswordNamespace[name] ?? name)
          .toLowerCase()
          .includes(searchTerm.toLowerCase())
    );
  }, [selectedRank, isFinalOnly, mhWildsLongswordNamespace, searchTerm]);
              ...
              <div key={name} className="border p-4 rounded shadow space-y-2">
                <div className="flex items-center">
                  <div className="font-semibold">
                    {mhWildsLongswordNamespace[name]}
                  </div>
                  {slots.length > 0 && (
