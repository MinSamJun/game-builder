...
import { mhWildsHammersData } from "@/data/mh-wilds";
import { NoResults } from "@container/common/no-results";
import { usePagination } from "@/hook/use-pageation";
import { Pagination } from "@infrastructure/common/pagenation";

export function HammerList({ searchTerm }: { searchTerm: string }) {
  const { getNamespaceData } = useI18n();

  const mhCommonNamespace = getNamespaceData("mh_common");
  const mhWildsHammerNamespace = React.useMemo(
    () => getNamespaceData("mhWilds_hammers") ?? {},
    [getNamespaceData]
  );
  ...
  const filteredList = React.useMemo(() => {
    return mhWildsHammersData.filter(
      ({ name, rank, rarity }) =>
        (!selectedRank || rank === selectedRank) &&
        (!isFinalOnly || rarity === 4 || rarity === 8) &&
        (mhWildsHammerNamespace[name] ?? name)
          .toLowerCase()
          .includes(searchTerm.toLowerCase())
    );
  }, [selectedRank, isFinalOnly, mhWildsHammerNamespace, searchTerm]);
