...
import { mhWildsDualbladesData } from "@/data/mh-wilds";
...
export function DualbladeList({ searchTerm }: { searchTerm: string }) {
  ...
  const mhWildsDualbladeNamespace =
    getNamespaceData("mhWilds_dualblades") ?? {};
  ...
  const filteredList = mhWildsDualbladesData.filter(
    ({ name, rank, rarity }) =>
      (!selectedRank || rank === selectedRank) &&
      (!isFinalOnly || rarity === 4 || rarity === 8) &&
      (mhWildsDualbladeNamespace[name] ?? name)
        .toLowerCase()
              ...
              <div key={name} className="border p-4 rounded shadow space-y-2">
                <div className="flex items-center">
                  <div className="font-semibold">
                    {mhWildsDualbladeNamespace[name]}
                  </div>
                  {slots.length > 0 && (
